...
class MultiHeadConv1D(nn.Module):
    def __init__(self, em_size: int=512, kernel_size_group: tuple=(1,3,5,7),\
        ...
        self.conv1d_list = nn.ModuleList([nn.Conv1d(in_channels=channels, out_channels=channels, kernel_size=k, stride=s, padding=p) for k, s, p in zip(self.kernel_size_group, self.stride_group, self.padding_group)])
    ...
    def forward(self, x):
        ...
        for h in range(self.num_heads):
            conv1d = self.conv1d_list[h]
            x_list.append(conv1d(x[h]))
